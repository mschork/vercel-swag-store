...
| Session store | `lib/session/store.ts` | never `"use cache"`; Upstash Redis over `fetch` with a 300 ms timeout, keyed by the session id from `cookies()`; `proxy.ts` mints the id with no I/O and runs only for a request without a valid one; an in-memory adapter when `KV_REST_API_URL` is unset; read once per render inside `<Suspense>` |
| Stock for a product | `lib/api/stock.ts` | never cached; drawn once per visitor by the first render or cart action that needs it and claimed in the session store, where the first write wins; surfaces read the store inside `<Suspense>` |
...
| Search results | `app/search/page.tsx`, `app/search/category/[slug]/page.tsx` | a category with no query is rewritten to a prerendered page per category; a query is dynamic via `searchParams`; each argument set is cached through `getProductsAtRequestTime` and `getFeaturedProductsAtRequestTime`, `'use cache: remote'`, because a request-time `"use cache"` read rarely hits on Vercel |
