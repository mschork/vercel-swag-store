...
| Search results | `app/search/page.tsx` | dynamic via `searchParams`; each argument set is cached through `getProductsAtRequestTime`, `'use cache: remote'`, because a request-time `"use cache"` read rarely hits on Vercel |
