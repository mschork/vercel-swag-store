...
- Decided in review, not in the spec: pnpm bumped to the latest stable (12.4.1) rather than a 10.x pin; the requirements kept out of the repository; a git-ignored `working/` folder for unredacted documents.
...
- The Vercel triangle is a trademark used here because the store is named "Vercel Swag Store"; `specs/decisions.md` records the choice.
...
- `getFeaturedProducts({ limit, min })` lives in the products lib and is reused by E07; `min: 6` is the required minimum, not an API count.
...
- Applied: gallery `sizes` fixed to the 528px column; an unknown product cached with the `minutes` profile instead of the catalogue lifetime (a reader noted every mistyped slug held a catalogue entry and a newly added product could keep returning 404).
- Answered: a reader quotes the docs line "with Cache Components, every dynamic route streams a static shell first" against the 404 claim. This page resolves the product before any boundary and the manifest shows no fallback shell (`fallback: null`, `compute: "blocking"`); `next start` returns 404 and Playwright asserts it. The live Vercel check waits for production.
...
An open redirect in `/api/draft-mode/disable`, found in the pre-release audit and reproduced on production:
