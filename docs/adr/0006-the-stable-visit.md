...
- **Derive a count from a hash of the product id.** Stable and free, and rejected: `the requirements` asks for stock "fetched from the provided API", and a hash is not fetched from anything.
...
- A render without a cookie costs one promotion call on every route and one stock call on a product page, for crawlers and link previews too. Superseded: such a render draws the whole catalogue in the layout's seed hole (`0007-the-session-store.md`).
