...
- The store needs `KV_REST_API_URL` and `KV_REST_API_TOKEN`. Without them an in-memory adapter serves one process, which is what a clone, CI and the unit tests get. State then lasts as long as the process.
