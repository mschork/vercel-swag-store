...
/**
 * Switches draft mode on for an editor (E17). `defineEnableDraftMode` accepts
 * only the short-lived secret the Studio's Presentation tool mints, and checks
 * it against Sanity with the read token, so a stranger cannot call this. In a
