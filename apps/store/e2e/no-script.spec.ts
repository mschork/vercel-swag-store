...
test.describe('without JavaScript', () => {
  ...
  test('the server keeps the visit it drew, under the one cookie the store sets', async ({
    page,
    context,
  }) => {
    await page.goto('/products')
    ...
    const kept = Object.keys((await readVisit(context))?.stock ?? {})
    expect(kept).toContain(await productIdOf(page))
  })
})
