 ...
 */
test('home renders the hero, the featured grid and at least six product cards', async ({ page }) => {
  await page.goto('/')
  await expect(page.getByRole('heading', { level: 1 })).toBeVisible()
  await expect(page.getByRole('link', { name: 'Shop the collection' })).toHaveAttribute('href', '/search')
