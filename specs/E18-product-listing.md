...
- The header's "Search" link gets a magnifying-glass glyph before its label: an inline SVG in the cart icon's stroke style, `aria-hidden`, no icon dependency. The glyph is inline and nudged onto the text, never a flex item: an inline-flex wrapper takes its baseline from the SVG and lifts the word above its neighbours. The label stays the accessible name. With three links the row is 2px too wide at 320px, so below sm the label is visually hidden and the glyph stands alone.
