...
The brief asks for "real-time stock availability", a stock indicator "fetched from the provided API", a quantity selector "respecting stock limits" and an Add to Cart button "disabled when the product is out of stock". All four still hold, and the last two become true for the first time, because a limit that is redrawn before the add lands was never a limit:
...
- It is always present, for a visitor as much as for a reviewer. The store is a demonstration and the button says so; hiding it behind an environment variable would make the one thing a reviewer wants to try the one thing they cannot reach.
