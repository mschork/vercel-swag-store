...
Branch: `epic/E13-search-gaps`. Depends on: E07, E09. Blocks: nothing. Ship only after E12 is otherwise ready; it must never delay the release.
...
Uses `after()` (work after the response), keeps the request path fast, writes only from the server, uses the AI SDK for analysis rather than chat, and closes a loop between storefront and content operations. All within the requirements's rules: the API stays the catalogue; Sanity holds editorial data.
