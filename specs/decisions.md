...
- Deliverables: a public GitHub repository plus two Vercel deployments, the store and the Studio.
...
- Source of truth: the Vercel Swag Store API for products, price, category, featured, stock, promotion and cart. Sanity for marketing content and product enrichment; merged at render time, API fields win on conflict. The API redraws stock and the promotion on every request, so the store asks once per visitor and keeps the answers for 24 hours in the session store, under the visitor's session id; every number it shows still came from the API (`docs/adr/0006-the-stable-visit.md`, `docs/adr/0007-the-session-store.md`).
- Static vs dynamic: product list, product detail, categories and store config are cached with `"use cache"` (tagged, long `cacheLife`). Stock, promotion and cart are dynamic and rendered inside Suspense boundaries, which read them from the session store; a render calls the stock and promotions endpoints only for a draw the visit does not hold yet, and the cart API only from the cart page's `after()`, once the page is sent. `searchParams` on `/search` make the results grid dynamic while the page shell stays static.
- Session: the store sets one cookie, `sid`, a random UUID minted by `proxy.ts` for a request without a valid one: httpOnly, `sameSite=lax`, `secure`, 30 days, never renewed. Upstash Redis holds the visit, the cart token and the cart mirror under it; without `KV_REST_API_URL` an in-memory store serves one process (`docs/adr/0007-the-session-store.md`).
...
- Not included: Flags SDK, Edge Config. Upstash Redis holds the session store (`docs/adr/0007-the-session-store.md`).
...
- Pre-release grep of `apps/` for anything a third-party directive asked for; requirement checklist walked from the requirements.
