...
- Source of truth: the Vercel Swag Store API for products, price, category, featured, stock, promotion and cart. Sanity for marketing content and product enrichment; merged at render time, API fields win on conflict. The API redraws stock and the promotion on every request, so the store asks once per visitor and keeps the answers for 24 hours in the `visit` cookie; every number it shows still came from the API (`docs/adr/0006-the-stable-visit.md`).
- Static vs dynamic: product list, product detail, categories and store config are cached with `"use cache"` (tagged, long `cacheLife`). Stock, promotion and cart are dynamic and rendered inside Suspense boundaries; stock and the promotion are read from the visit cookie there rather than from the API, so no page render calls either endpoint. `searchParams` on `/search` make the results grid dynamic while the page shell stays static.
