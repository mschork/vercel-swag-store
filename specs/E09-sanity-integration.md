...
- **Cart** (E06 revisit): the same favourites row, under the cart. Empty, it follows the home page rule; with lines in it, the products already in the cart are excluded and the ranking is read one deeper per line so the row still fills. The visitor's sold-out products are excluded the same way (E19), so this row offers only what can be bought, and each card carries an Add to Cart for one. Only the exclusion is dynamic.
