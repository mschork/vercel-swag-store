...
Shared with E07. Props: `product`, `priority?`. Renders `next/image` with the product's first image and `sizes="(min-width: 1024px) 25vw, (min-width: 768px) 33vw, 50vw"`, name, `formatPrice(price)`, wrapped in a `Link` to `/products/[slug]`. Whole card is the link; name and price sit in a pill in the bottom-left [assumption, echoing the reference design in the brief]. Prefetch default.
