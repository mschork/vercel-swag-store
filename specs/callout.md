...
- **In production this would be a session ID and a store; here the cookie avoids a datastore the brief does not ask for** (E19, E21). A real shop keeps visitor state on the server and gives the browser a small random ID. Doing that here needs three things: a key-value store such as Redis, the proxy minting the ID on the first request, because a page cannot set a cookie, and the stock hole and the visit route reading the store where they read the cookie today. It would buy a lot. The cookie shrinks from about 1.1 KB on every request to a few dozen bytes, the cap of about 135 products goes away, and a visitor can no longer edit their own stock. It would also remove the one part of E21 that no real shop needs: a render that draws stock for a first-time visitor could store the number under the ID itself, so the browser would not have to hand the shown number back to `POST /api/visit`, and the open call would not have to wait for the stock hole to report. The store does not do it because of what it costs in this setting. It is a second service and a dependency in a repo that keeps its list short, a reviewer who clones the repo would need credentials for it before the store runs, the proxy would run on every request, and each dynamic hole would make a network read where it parses a cookie for free. None of that demonstrates `"use cache"`, Suspense or Server Actions, which is what the brief grades. So the cookie is a choice for the assignment, not a recommendation for production, and the hand-back is what it costs to keep that choice and still put the first visitor's stock in the HTML. `docs/adr/0006-the-stable-visit.md` lists the session ID among the considered options. How the proxy would pass the ID to a partially prerendered page is not verified; it would want a spike first.
