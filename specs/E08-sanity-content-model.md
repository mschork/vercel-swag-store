...
- `siteSettings` (singleton): `storeName`, `seoTitle`, `seoDescription`, `ogImage` (image with alt), `socialLinks[]` `{ label, url }`, `footerText`, `productPage { aboutHeading, careHeading, testimonialsHeading, faqHeading }` (the headings on a product page; each empty field falls back to the wording the store ships).
- `homePage` (singleton): `hero { headline, description, image (image with alt and hotspot) }`, `featured { heading, linkLabel }` (the copy over the featured grid; empty fields fall back to “Featured” and “View all”), `favourites { heading }` (the copy over the row that testimonials rank; empty, it says “People’s favourites”).
- `checkoutPage` (singleton): `title`, `body` (Portable Text), `continueShoppingLabel`.
- `testimonial`: `person`, `role`, `photo` (image with required alt and hotspot), `quote`, `products[]` (references to `product`), `consent` (boolean, must be true to publish, enforced by validation), `publishedAt`.
...
Runs the sync, then writes demonstration content: the three singletons with the store's current fallback copy, two enriched products (hoodie and backpack) with a description and care text, two testimonials, and four FAQs. Placeholder copy, listed in `specs/improvements.md`. Idempotent.
